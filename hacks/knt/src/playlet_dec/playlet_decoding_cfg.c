...
#define USAGE  "spc_playlet_dec usage:\n" \
    ...
    "\t--xml        | -x <output xml playlet filepath>\n" \
	"\t--version    | -v <version>\n" \
	"\t--help       | -h <help>\n"
...
playlet_dec_cfg_t *playlet_dec_cfg_init(int argc, char **argv) {
    ...
    if (NULL == (cfg = config_parse_args(argc, argv, short_options, long_options))) {
        LOG_ERROR("playlet_dec_load_cfg() failed - cannot parse args to initialize config structure.");
        return NULL;
    }
...
int check_config(void *cfg) {
    ...
    return ((((playlet_dec_cfg_t *) cfg)->cpu_mem != DEFAULT_CPU_MEM_VALUE)
            && (((playlet_dec_cfg_t *) cfg)->rom != DEFAULT_ROM_VALUE)
            && (((playlet_dec_cfg_t *) cfg)->xml != DEFAULT_XML_VALUE));
}
/******************************************************************************/
...
void *init_config(void) {
    ...
    if (NULL == (cfg = (playlet_dec_cfg_t *) malloc(sizeof(playlet_dec_cfg_t)))) {
...
void print_config(void *cfg) {
    LOG_INFO("Script encoding parameters:\n"
             ...
             "\toriginal rom:         %s\n"
             "\toutput file:         %s\n",
            ((playlet_dec_cfg_t * ) cfg)->cpu_mem,
            ((playlet_dec_cfg_t * ) cfg)->rom,
            ((playlet_dec_cfg_t * ) cfg)->xml);
}
/******************************************************************************/
void uinit_config(void *cfg) {
    if (cfg)
        free(((playlet_dec_cfg_t *) cfg));
}
